...
`ParserDiagnostics`, and one immutable `DatasetSemanticContext`.
...
`NONE` is represented explicitly by identity bias/scale and zero counters; it is never encoded as
an absent object. Dataset construction fails if a valid snapshot cannot be selected.
